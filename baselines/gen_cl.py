...
assert dataset in ('email-dnc', 'email-enron', 'email-eucore', 'facebook-links', 'coath-dblp')
...
logger.add(join(rootpath, logpath, f'cl_{dataset}_{mode}_timing.log'), mode='w')

loaded = load_data(dataset)
graphs = [g for _, g in loaded]

for t, graph in enumerate(graphs):
    params = fit_timer(chung_lu, logger)(graph, directed=False)
