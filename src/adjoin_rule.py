...
from src.decomposition import ancestor, common_ancestor, propagate_descendants
...
def mutate_rule_domestic(grammar: VRG, u: int, v: int, time: int, mode: str = 'add'):
    ...
    """
    parent_idx, parent_rule, ancestor_nodes = common_ancestor({u, v}, grammar)
    ancestor_u = ancestor_nodes[u]
    ancestor_v = ancestor_nodes[v]

    parent_rule.edit_dist += 1  # cost of adding/removing an edge
    ...
    if mode == 'add':
        ...
        for ancestor_x in (ancestor_u, ancestor_v):
            if 'label' in parent_rule.graph.nodes[ancestor_x]:  # if we modified a nonterminal, propagate that change downstream
                parent_rule.edit_dist += 1  # cost of relabeling a node
                parent_rule.graph.nodes[ancestor_x]['label'] += 1  # one more edge incident on this symbol
                propagate_descendants(ancestor_x, parent_idx, grammar, time)
    elif mode == 'del':
        try:
            parent_rule.graph.remove_edge(ancestor_u, ancestor_v)
        except Exception:
...
def mutate_rule_diplomatic(grammar: VRG, u: int, v: int, time: int):
    """
        ...
        Returns:
            The grammar with the mutated rule in it.
    """
    parent_idx, parent_rule, ancestor_u = ancestor(u, grammar)
    ancestor_v = chr(ord(max(parent_rule.graph.nodes())) + 1)
    ...
    if 'label' in parent_rule.graph.nodes[ancestor_u]:  # if we modified a nonterminal, propagate that change downstream
        parent_rule.edit_dist += 1  # cost of relabeling a node
        parent_rule.graph.nodes[ancestor_u]['label'] += 1  # one more edge incident on this symbol
        propagate_descendants(ancestor_u, parent_idx, grammar, time)

    grammar.cover[v] = parent_idx  # rule_source now points to the same location in the rule_tree for u and v
    parent_rule.alias[v] = ancestor_v
